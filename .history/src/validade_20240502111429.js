...
function validade(number) {
  if (number.length !== 11) { return 'Array com tamanho incorreto.'; } if (number.some((num) => num < 0 || num > 9)) { return 'não é possível gerar um número de telefone com esses valores'; }
  const frequency = {};
  let repeated = false;
